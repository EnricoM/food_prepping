...
class _EditRecipeScreenState extends ConsumerState<EditRecipeScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: const BackAwareAppBar(title: Text('Edit recipe')),
      body: SafeArea(
  ...
  Future<void> _saveRecipe() async {
    ...
    try {
      ...
      final updatedRecipe = widget.recipe.copyWith(
        ...
        prepTimeMinutes: _durationFromMinutes(_prepMinutesController.text.trim())?.inMinutes,
        cookTimeMinutes: _durationFromMinutes(_cookMinutesController.text.trim())?.inMinutes,
        totalTimeMinutes: _computeTotalTime()?.inMinutes,
      ...
      // Navigate back and refresh the detail screen
      Navigator.of(context).pop();
      
      // Get updated recipe and navigate to detail screen
      final updated = repository.getById(widget.recipe.id);
      if (updated != null && mounted) {
        RecipeNavigation.pushRecipeDetails(context, updated);
      }
    } catch (error) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
